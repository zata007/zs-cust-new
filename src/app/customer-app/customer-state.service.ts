...
import {
  IBusinessLocData, IEssentialProductData, IMenuData, IRestaurantData, Marker
...
export class CustomerStateService {
  ...
  currenLocationRestaurantData$ =
    this.currentLocationRestaurantData.asObservable();

  locationSelectionCompleted$ = new Subject<boolean>();
  directionResults$ = this.socket.fromEvent('getDirectionsResult');
  pitstopOnEdge$ = this.socket.fromEvent<{
    isLocationOnEdge: boolean;
    pitstop: number[];
    pitstopId: string;
  }>('checkLocationOnEdgeResult');
  orderId: string;
  ...
  initState() {
    ...
    this.currentLocationRestaurantData.next([
      ...this.currentLocationRestaurantData.value,
    ]);
  }
  ...
  updateSelectedRoute(selectedPolyline: {
    color: string;
    path: { lat: number; lng: string }[];
    encodedString: string;
  }) {
    const selectedRouteIndex = this.directionResults.findIndex(
      (i) => i.overview_polyline['points'] === selectedPolyline.encodedString
    );
